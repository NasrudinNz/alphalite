...
uses Registry, Windows, Graphics, SysUtils,
  ...
  acntUtils{$IFNDEF ALITE}, acArcControls, acPathDialog, sColorDialog, sStoreUtils{$ENDIF};
...
begin
  ...
  try
    SplashScreenServices.AddPluginBitmap('AlphaControls 2017', Bmp.Handle, False,
{$IFNDEF ALITE}
...
{$IFNDEF ALITE}
  ...
  RegisterComponents('AlphaTools', [
    ...
    TsAlphaHints, TsTitleBar, TsFloatButtons
  {$IFDEF USEHINTMANAGER}
    , TsHintManager
  {$ENDIF}
    ]);

  RegisterComponents('AlphaArcControls', [TsMeter, TsArcGauge, TsArcPreloader, TsArcHandle, TsArcDial, TsRoundBtn]);
